...
export class XRExampleBase {
  constructor(
    ...
    if (typeof navigator.XR === "undefined") {
      this.showMessage(
        "No WebXR API found, usually because the WebXR polyfill has not loaded"
      );
      return;
    }
  ...
  _startSession() {
    let sessionInitParameters = {
      ...
      type: this.createVirtualReality
        ? XRSession.REALITY
        : XRSession.AUGMENTATION,
    ...
    this.display
      .requestSession(sessionInitParameters)
      .then(session => {
  ...
  */
  ...
  _handleFrame(frame) {
    ...
    const headPose = frame.getDisplayPose(
      frame.getCoordinateSystem(XRCoordinateSystem.HEAD_MODEL)
    );
    ...
    if (this.requestedFloor === false) {
      ...
      frame
        .findFloorAnchor("first-floor-anchor")
        .then(anchorOffset => {
          if (anchorOffset === null) {
            console.log("could not find the floor anchor");
            const headCoordinateSystem = frame.getCoordinateSystem(
    ...
    for (let anchoredNode of this.anchoredNodes) {
      this.updateNodeFromAnchorOffset(
        frame,
        anchoredNode.node,
...
*/
export class ThingsOnSurfacesApp extends XRExampleBase {
...
function loadObj(baseURL, geometry) {
  return new Promise(function(resolve, reject) {
    ...
    const mtlName =
      geometry.split(".")[geometry.split(":").length - 1] + ".mtl";
...
*/
global.throttle = function(func, wait, leading = true, trailing = true) {
...
global.throttledConsoleLog = throttle((...params) => {
  console.log(...params);
}, 1000);
